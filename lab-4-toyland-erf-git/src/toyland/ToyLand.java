...
public class ToyLand {
    ...
    public void playTime() {
        IToy nextToy = activeToys.remove(0);
        nextToy.play(getRandomTime());
        if (nextToy.isRetired()) {
            retiredToys.add(nextToy);
        } else {
            activeToys.add(nextToy);
        }
        if (activeToys.size() > 0) {
            playTime();
        }
    }
    ...
    public void displayStatistics() {
        ...
        for (IToy t : retiredToys) {
            System.out.println("\t" + t.toString());
            totalToys++;
            totalHappiness += t.getHappiness();
            totalWear += t.getWear();
        }
        System.out.println("Total Toys: " + String.valueOf(totalToys));
        System.out.println("Total Happiness: " + String.valueOf(totalHappiness));
        System.out.println("Average wear: " + String.valueOf(totalWear / totalToys));
    }
