...
public abstract class IMDB {
    ...
    public IMDB(boolean small) throws FileNotFoundException {
        ...
        // TODO Activity 1
        // consume the header line and discard it
        in.nextLine();
        while (in.hasNext()) {
            ...
            if (!fields[4].equals(IS_ADULT)) {
                // create the Movie for this line using the appropriate fields
                // append the Movie to the back of this.movieList
                movieList.add(Movie.createMovie(fields[0], fields[1], fields[2], fields[5], fields[7], fields[8]));
            }
        }
        in.close();

        // print the total number of movies read (not including adult movies)
    ...
    public void convertMovieListToMap() {
        ...
        // Adds all the movies in movieList to movieMap
    ...
    public void processRatings() throws FileNotFoundException {
        ...
        // read the ratings in and use the movieMap to find the movie and update its rating.
        in.nextLine();
        while (in.hasNext()) {
            String line = in.nextLine();
            String[] fields = line.split("\t");
            if (movieMap.containsKey(fields[0])) {
                movieMap.get(fields[0]).setRating(new Rating(fields[0], Double.parseDouble(fields[1]), Integer.parseInt(fields[2])));
            }
        }
        in.close();
